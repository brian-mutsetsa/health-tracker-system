...
class PatientSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Patient
        fields = ['id', 'patient_id', 'name', 'surname', 'condition', 'status',
                  'date_of_birth', 'age', 'gender', 'id_number', 'phone_number',
                  'district', 'home_address',
...
class PatientRegistrationSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Patient
        fields = ['patient_id', 'name', 'surname', 'condition', 'password',
                  'date_of_birth', 'gender', 'id_number', 'phone_number', 'pin',
                  'district', 'home_address',
                  'emergency_contact_name', 'emergency_contact_phone', 'emergency_contact_relation',
                  'weight_kg', 'blood_pressure_systolic', 'blood_pressure_diastolic',
                  'blood_glucose_baseline', 'medical_history', 'medications', 'allergies',
...
class PatientListSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Patient
        fields = ['id', 'patient_id', 'name', 'surname', 'condition', 'status', 'age',
                  'gender', 'phone_number', 'district',
                  'last_risk_level', 'last_risk_color', 'last_checkin', 'updated_at', 'total_checkins']
...
class CheckInCreateSerializer(serializers.Serializer):
    ...
    def validate_answers(self, value):
        """Validate answers contains symptom questions"""
        if not isinstance(value, dict):
            raise serializers.ValidationError("Answers must be a dictionary")
        if len(value) < 11 or len(value) > 12:
            raise serializers.ValidationError(f"Expected 11-12 answers, got {len(value)}")
        return value
